...
def expand_product_data(product_data):
    """
    Attempt to expand product data by fetching the full product page
    This is useful for products where we only have a URL
    """
    if not product_data or not product_data.get('product_link'):
        return product_data
        
    # Skip if we already have most of the data
    if (product_data.get('brand') and product_data.get('product_name') and 
        product_data.get('price') and product_data.get('product_image')):
        return product_data
        
    try:
        product_url = product_data['product_link']
        logger.info(f"Expanding data for product: {product_url}")
        
        # Fetch the product page
        response = get_page_with_retry(product_url)
        ...
        # Try to extract data from API or JSON-LD first
        api_data = extract_api_data(html_content)
        if api_data and len(api_data) > 0:
            first_product = api_data[0]
            # Update missing fields
            if not product_data.get('brand') and first_product.get('brand'):
                product_data['brand'] = first_product['brand']
            if not product_data.get('product_name') and first_product.get('name'):
                product_data['product_name'] = first_product['name']
            if not product_data.get('price') and first_product.get('price'):
                price_str = first_product['price']
                ...
                if price_matches:
                    try:
                        ...
                        pass
            if not product_data.get('product_image') and first_product.get('image'):
                image_url = first_product['image']
                if not image_url.startswith('http'):
                    image_url = f"https://www.sephora.com{image_url}"
                product_data['product_image'] = image_url
            return product_data
        ...
        if not product_data.get('brand'):
            brand_elements = [
                soup.select_one('[data-at="brand_name"]'),
                soup.select_one('[data-comp="BrandName"]'),
                soup.select_one('.css-cjz2sh'),
                soup.select_one('.product-brand')
            ]
            for el in brand_elements:
                if el and el.text.strip():
                    product_data['brand'] = el.text.strip()
                    break
                    
        # Product name
        if not product_data.get('product_name'):
            name_elements = [
                soup.select_one('[data-at="product_name"]'),
                soup.select_one('[data-comp="DisplayName"]'),
                soup.select_one('.css-1n1cap4'),
                soup.select_one('.product-name'),
                soup.select_one('h1')
            ]
            for el in name_elements:
                if el and el.text.strip():
                    product_data['product_name'] = el.text.strip()
                    break
                    
        # Price
        if not product_data.get('price'):
            price_elements = [
                soup.select_one('[data-at="price"]'),
                soup.select_one('[data-comp="Price"]'),
                soup.select_one('.css-18jtttk'),
        ...
        # Image
        if not product_data.get('product_image'):
            image_elements = [
                soup.select_one('[data-at="product_image"] img'),
                soup.select_one('[data-comp="ProductImage"] img'),
                soup.select_one('.css-1rovmyu img'),
        ...
        if not product_data.get('rating'):
            rating_elements = [
                soup.select_one('[data-at="ratings_reviews"]'),
                soup.select_one('[data-comp="StarRating"]'),
                soup.select_one('.css-14lnvpw')
            ]
            for el in rating_elements:
                if el and el.text.strip():
                    rating_str = el.text.strip()
        ...
        logger.error(f"Error expanding product data: {e}")
...
def crawl_category_page(category_url, max_pages=10):
    ...
    for page in range(1, max_pages + 1):
        ...
        selector_patterns = [
            ...
            # Even more selectors for greater coverage
            '[data-at="sku_item"] a',
            '.css-1qe8tjm a',
            '.css-b8rwz8 a',
            'a[data-at="sku_item_link"]',
            '.product-tile a'
        ]
                ...
                if product_links:
                    ...
                    for link in product_links:
                        ...
                        if processed_product and processed_product['product_link'] not in products_seen:
                            # Try to expand the product data to get more details
                            expanded_product = expand_product_data(processed_product)
                            products_seen[processed_product['product_link']] = expanded_product
                            all_products.append(expanded_product)
                            products_found_on_page = True
        ...
        for idx, product_el in enumerate(product_elements):
            try:
                ...
                if processed_product:
                    ...
                    if product_link not in products_seen:
                        # Consider expanding product data for incomplete entries
                        if not processed_product.get('brand') or not processed_product.get('price'):
                            processed_product = expand_product_data(processed_product)
    ...
    # Expand data for any products with missing information
    expanded_products = []
    for product in all_products:
        if not product.get('brand') or not product.get('price'):
            expanded_products.append(expand_product_data(product))
        else:
    ...
    return final_productsIGNORECASE)),
                    soup.find_all(string=re.compile(r'no products', re.IGNORECASE))
                ]
